...
function mergePersistedState(persistedState: unknown, currentState: AppState): AppState {
  ...
  return {
    ...currentState,
    settings,
    params: isRecord(persisted.params) ? { ...currentState.params, ...persisted.params } : currentState.params,
    dismissedCodexCliPrompts: Array.isArray(persisted.dismissedCodexCliPrompts)
      ? persisted.dismissedCodexCliPrompts.filter((item): item is string => typeof item === 'string')
      : [],
    appMode: persisted.appMode === 'editor' ? 'editor' : 'gallery',
    prompt: settings.persistInputOnRestart && typeof persisted.prompt === 'string' ? persisted.prompt : '',
    inputImages: settings.persistInputOnRestart ? normalizeInputImages(persisted.inputImages) : [],
    maskDraft: settings.persistInputOnRestart ? normalizeMaskDraft(persisted.maskDraft) : null,
    maskEditorImageId: null,
    favoriteCollections,
    defaultFavoriteCollectionId: resolveDefaultFavoriteCollectionId(favoriteCollections, persisted.defaultFavoriteCollectionId),
...
export const useStore = create<AppState>()(
  persist(
    (set, get) => ({
      // Mode
      appMode: 'gallery',
      setAppMode: (appMode) => set({ appMode, selectedTaskIds: [], selectedFavoriteCollectionIds: [] }),
      // Settings
...
export async function initStore() {
  ...
  for (const task of tasks) addTaskReferencedImageIds(referencedIds, task)
  ...
  if (restoredInputImages.length !== state.inputImages.length || restoredInputImages.some((img, index) => img.dataUrl !== state.inputImages[index]?.dataUrl)) {
